...
export default class Favourite extends Component {
    ...
    componentDidMount() {
        const movieData = JSON.parse(localStorage.getItem("movies")||"[]")// Ensure movies.results exists
    ...
    render() {
        const { movies, genres, currgen, currText, limit, currPage } = this.state;
        let genreids = {28:'Action',12:'Adventure',16:'Animation',35:'Comedy',80:'Crime',99:'Documentary',18:'Drama',10751:'Family',14:'Fantasy',36:'History',
                           27:'Horror',10402:'Music',9648:'Mystery',10749:'Romance',878:'Sci-Fi',10770:'TV',53:'Thriller',10752:'War',37:'Western'};
                        ...
                        if (currgen !== "All Genres") {
                            filterarr = filterarr.filter((movieobj) => genreids[movieobj.genre_ids[0]] === currgen);
                        }
                    ...
                    <div className="row ">
                        <div className='col-lg-3 col-sm-12' >
                            <ul className="list-group favourites-genres">
                                {genres.map((genre) => (
                                    currgen === genre ?
                                    <li key={genre} className="list-group-item" style={{background:'#3f51b5',color:'white',fontWeight:'bold'}}>{genre}</li> :
                                    <li key={genre} className="list-group-item" style={{background:'white',color:'#3f51b5'}} onClick={()=>this.handleGenreChange(genre)}>{genre}</li>
                                ))}
                            </ul>
                        </div>
                        <div className='col-lg-9  favourites-table  col-sm-12'>
    ...
    // Render all movies when filterarr is empty
    movies.map((movieObj) => (
        <tr key={movieObj.id}>
            <td><img src={`https://image.tmdb.org/t/p/original${movieObj.backdrop_path}`} alt={movieObj.title} style={{width:'5rem'}}/> {movieObj.original_title}</td>
            <td>{genreids[movieObj.genre_ids[0]]}</td>
